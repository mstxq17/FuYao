...
def ksubdomain(target, date):
    ...
    time.sleep(1)
    if config['system'] == 'mac':
        os.system('./core/plus/mac/ksubdomain enum --band 1G -dl %s --skip-wild --silent --only-domain --level %s --retry 1 --output logs/ksubdomain/%s'%(target, config['level'], date + '_ksubdomain.txt'))
